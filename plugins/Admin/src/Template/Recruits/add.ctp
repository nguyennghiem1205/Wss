...
<div class="row">
    <div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
        <h1 class="page-title txt-color-blueDark">
            ...
            <?= __('system');?>
            <span>&gt;
                <?= $this->Html->link(__('list of recruitment news'), ['action' => 'index'])?>
            </span>
        </h1>
    </div>
</div>
        ...
        <div class="jarviswidget jarviswidget-sortable jarviswidget-color-darken" id="wid-id-4" data-widget-editbutton="false" data-widget-custombutton="false" role="widget" style="">
            ...
            <div role="content">
                ...
                <div class="widget-body no-padding">
                    <?= $this->Form->create($recruit, ['id' => 'smart-form-register', 'class' => 'smart-form', 'type' => 'file']) ?>
                    <fieldset>
                        ...
                        <section>
                            ...
                            <label class="input">
                                <?= $this->Form->input('position', ['required' => false, 'placeholder' => __('Position')]); ?>
                            </label>
                        </section>

                        <section>
                            <label for="number" class="label"><?= __('Number') ?></label>
                            <label class="input">
                                <?= $this->Form->input('number', ['required' => false, 'placeholder' => __('Number')]); ?>
                            </label>
                        </section>

                        <section>
                            <label for="deadline" class="label"><?= __('Set deadline')?></label>
                            <label class="input">
                                <?= $this->Form->input('deadline', ['required' => false, 'placeholder' => date("Y-m-d H:i"), 'id' => 'datetimepicker', 'type' => 'text', 'value' => isset($recruit->deadline) ? date("Y-m-d H:i", strtotime($recruit->deadline)) : date("Y-m-d H:i")]); ?>
                            </label>
                        </section>

                        <section>
                            <label for="image" class="label"><?= __('Image') ?></label>
                            <?= ($recruit->image) ? $this->Core->image('Recruits/'.$recruit->image, 50, 50) : '' ?>
                            <label class="input">
                        ...
                        <section>
                            <label for="file" class="label"><?= __('File') ?><p id="titleFile"><?php if($recruit->file) echo ($recruit->file) ?></p></label>
                            <label class="input">
...
<script>
    ...
    var cID = <?= ($recruit->id) ? json_decode($recruit->id) : -1 ?>;
    inputImage = $('#image');
        ...
        if(cID < 0){
            ...
            if (confirm('Do you want to delete this image?')) {
                ...
                ({
                    type: "POST",
                    dataType: 'json',
                    url: baseUrl + 'pages/ajaxDeleleImage',
                    data: 'cid=' + cID,
        ...
        if(cID < 0){
            ...
            if (confirm('Do you want to delete this file?')) {
                ...
                ({
                    type: "POST",
                    dataType: 'json',
                    url: baseUrl + 'pages/ajaxDeleleFile',
                    data: 'cid=' + cID,
                    success: function (data) {
                        alert("Delete success!");
                        $("#titleFile").fadeOut();
